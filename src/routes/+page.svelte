<script>
	...
	async function changeRoom(roomId) {
		...
		if (!room) return;
		currentRoomDetails.id = roomId;
		currentRoomDetails.name = room.name;
		currentRoomDetails.messages = [];
	...
	async function fetchMembers(roomId) {
		...
		for (let member of members) {
			...
			const memberUser = client.getUser(member.userId);
			const presenceState = await client.getPresence(member.userId);
			currentRoomDetails.members[member.userId] = { displayName: memberUser.displayName, presence: presenceState.presence };	
		}
	...
	onMount(async () => {
		...
		//console.log((await client.roomInitialSync(currentRoomDetails["id"])));
	});
